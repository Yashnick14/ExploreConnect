...
export const getPlaces = async (req, res) => {
  try {
    ...
    if (isAdmin === "true") {
      ...
      const isMember = user?.membership?.isMember === true;

      if (!isMember) {
        // anonymous OR normal registered users
        filter.exclusive = { $ne: true };
      }
    }
...
export const getLatestPlaces = async (req, res) => {
  try {
    ...
    if (isAdmin === "true") {
      ...
      if (!isMember) {
        // Non-members / anonymous cannot see exclusive
        filter.exclusive = { $ne: true };
      }
    }

    const places = await Place.find(filter).sort({ createdAt: -1 }).limit(4);

    res.status(200).json({ success: true, data: places });
  } catch (err) {
    console.error("❌ getLatestPlaces error:", err.message);
    res.status(500).json({ success: false, message: "Server error" });
  }
};
