...
const Places = () => {
  ...
  const { user, loadingUser, loadUserFromStorage } = useAuthStore();
  ...
  // ✅ Hydrate user from sessionStorage when component mounts
  useEffect(() => {
    loadUserFromStorage();
  }, [loadUserFromStorage]);

  // ✅ Fetch places after auth is resolved
  useEffect(() => {
    if (loadingUser) return;
    fetchPlaces(); // backend decides who sees what
  }, [loadingUser, user?._id, fetchPlaces]);

  // ✅ Apply category/district filters only
  useEffect(() => {
    let result = [...places];
    ...
    <div className="relative min-h-screen p-3 sm:p-4 md:p-5 pt-16 sm:pt-20 md:pt-24">
      ...
      >
        ...
        <div className="p-4 sm:p-4 md:p-6 h-full overflow-y-auto">
          ...
          <button
            className="mt-4 px-4 py-2.5 w-full text-white bg-gray-900 rounded-lg hover:bg-gray-800 text-sm font-medium"
            onClick={() => {
