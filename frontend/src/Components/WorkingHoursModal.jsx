...
export const DEFAULT_WEEKLY = [
  ...
  { key: "Wed", label: "Wednesday", isOpen: true,  open: "09:00", close: "17:00" },
  { key: "Thu", label: "Thursday",  isOpen: true,  open: "09:00", close: "17:00" },
  { key: "Fri", label: "Friday",    isOpen: true,  open: "09:00", close: "17:00" },
  { key: "Sat", label: "Saturday",  isOpen: false, open: "09:00", close: "17:00" },
];
...
const StepperTime = ({ value, onChange, disabled }) => {
  ...
  useEffect(() => { if (value) setParts(partsFrom24(value)); }, [value]);

  const commit = (p) => { setParts(p); onChange && onChange(partsTo24(p)); };

  const bumpHour = (dir) => {
    let { hour12, minute, ampm } = parts;
    if (dir === 1) {
      if (hour12 === 11) { hour12 = 12; ampm = ampm === "AM" ? "PM" : "AM"; }
      else if (hour12 === 12) hour12 = 1;
      else hour12 += 1;
    } else {
      if (hour12 === 12) { hour12 = 11; ampm = ampm === "AM" ? "PM" : "AM"; }
  ...
  const bumpMinute = (dir) => {
    let { hour12, minute, ampm } = parts;
    let m = minute + dir * 5;
    if (m >= 60) { m -= 60; bumpHour(1); return; }
    if (m < 0)   { m += 60; bumpHour(-1); return; }
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl">
        ...
        <div className="flex items-start justify-between px-6 pt-5">
          ...
          <button type="button" onClick={onClose} className="text-gray-500 hover:text-gray-800 p-2">✕</button>
        </div>

        {/* Body */}
        <div className="px-6 pb-6 mt-4 space-y-3">
          {local.map((d, idx) => (
            <div key={d.key} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              {/* Left: day + switch */}
              <div className="flex items-center gap-3 w-full sm:w-auto">
                ...
                <button
                  ...
                  className={`relative inline-flex h-5 w-10 items-center rounded-full transition ${d.isOpen ? "bg-indigo-600" : "bg-gray-300"}`}
                >
                  <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition ${d.isOpen ? "translate-x-5" : "translate-x-1"}`} />
                </button>
                <span className={`text-sm ${d.isOpen ? "text-indigo-700" : "text-gray-500"}`}>{d.isOpen ? "Open" : "Closed"}</span>
              </div>
          ...
          <div className="flex justify-end gap-3 pt-4">
            <button type="button" onClick={onClose} className="h-10 px-4 rounded-md border bg-white text-gray-700 hover:bg-gray-50">Cancel</button>
            <button
              type="button"
