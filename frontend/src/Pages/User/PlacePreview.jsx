...
const pickRow = (weekly, code) =>
  Array.isArray(weekly)
    ? weekly.find((d) => d.day === code || d.key === code || d.label === LABEL[code]) || null
    : null;
...
const nextOpenInfo = (weekly) => {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    if (i === 0) {
      if (nowMin >= openMin && nowMin <= closeMin) return `Closes at ${to12h(row.close)}`;
...
const mmFromHHMM24 = (hhmm) =>
  (/^\d{2}:\d{2}$/.test(hhmm) ? hhmm.split(":").map(Number).reduce((h, m) => h * 60 + m) : null);
...
const PlacePreview = () => {
  ...
  useEffect(() => {
    loadUserFromStorage?.();
    const unsub = onAuthStateChanged(getAuth(), (fbUser) => setIsAnonymous(!!fbUser?.isAnonymous));
    return () => unsub();
  }, [loadUserFromStorage]);
  ...
  const weekly = useMemo(() => {
    ...
    try { if (typeof w === "string") w = JSON.parse(w); } catch {}
    return w;
  }, [place]);
  ...
  const handleRegisterSubmit = async (payload) => {
    ...
    if (Array.isArray(weekly) && weekly.length === 7) {
      ...
      if (!range) return toast.error("Working hours unavailable. Please contact the place.");
      [openMin, closeMin] = range;
    }
    if (openMin == null || closeMin == null) return toast.error("Invalid opening hours configured.");
    if (userTimeMin < openMin || userTimeMin >= closeMin) {
      const nice = (m) => {
        const h = Math.floor(m / 60), mi = m % 60, ampm = h >= 12 ? "PM" : "AM", hr12 = h % 12 || 12;
        return `${hr12}:${String(mi).padStart(2, "0")} ${ampm}`;
      };
      return toast.error(`Closed at that time. Open window is ${nice(openMin)} – ${nice(closeMin)}.`);
    }
    ...
    if (success) { toast.success("Registration submitted"); setShowRegister(false); }
                  ...
                  {details.map((d, i) => (
                    <div key={i} className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-0.5 font-medium text-gray-900">{d.value}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {hasCoords && (
              <section>
                ...
                <div className="px-4 sm:px-6">
                  <PlaceMap dest={{ lat, lng }} name={place.name} height={320} zoom={13} />
                </div>
                  ...
                  {reviewsData.map((r, idx) => (
                    <div key={idx} className="py-6">
                      <div className="flex items-center gap-4">
                        ...
                        />
                        <div>
                          <p className="text-[15px] font-semibold text-gray-900">{r.name}</p>
                      ...
                      <div className="mt-4">
                        ...
                        <p className="mt-4 text-[15px] leading-relaxed text-gray-600">{r.text}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`transition-transform ${hoursOpen ? "rotate-180" : ""}`}>▲</span>
                </div>
              </button>

              {Array.isArray(weekly) && (
                <div className="px-5 pb-2 -mt-2">
                  <p className="text-[11px] text-gray-600">{nextOpenInfo(weekly)}</p>
                </div>
              )}

              <div className={`grid transition-all duration-300 ${hoursOpen ? "max-h-[480px] opacity-100" : "max-h-0 opacity-0"} overflow-hidden`}>
                  ...
                  {Array.isArray(weekly) && weekly.length === 7 ? (
                    <ul className="divide-y divide-gray-200 rounded-xl overflow-hidden border border-gray-200">
                      {ORDER.map((code) => {
                        ...
                        const timeText = dayOpen ? `${to12h(row.open)} – ${to12h(row.close)}` : "Closed";
                        return (
                          <li key={code} className={`flex items-center justify-between px-4 py-2.5 ${isToday ? "bg-gray-50" : "bg-white"}`}>
                            <div className="flex items-center gap-2">
                              <span className={`inline-block w-1.5 h-1.5 rounded-full ${dayOpen ? "bg-emerald-500" : "bg-rose-500"} ${isToday ? "" : "opacity-70"}`} />
                              <span className={`text-[13px] ${isToday ? "font-semibold text-gray-900" : "text-gray-700"}`}>{LABEL[code]}</span>
                              {isToday && <span className="ml-2 text-[10px] px-2 py-0.5 rounded-full bg-black/5 text-gray-700">Today</span>}
                            </div>
                            <span className={`text-[13px] tabular-nums ${dayOpen ? "text-gray-900 font-medium" : "text-rose-600 font-medium"}`}>{timeText}</span>
                          </li>
                        );
                      })}
                    </ul>
                    ...
                    <div className="p-3 rounded-xl border border-gray-200 bg-white">
                      ...
                      <p className="text-sm font-medium text-gray-900">{place.workingHours || "Not provided"}</p>
                    </div>
                  )}
                </div>
              </div>
